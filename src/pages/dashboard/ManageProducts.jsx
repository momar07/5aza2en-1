...
const categories = ['مقاومة للحريق','خزائن بنوك','خزائن منازل','خزائن مجوهرات','خزائن أرضية','أخرى']
...
export default function ManageProducts() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.name || !form.price || !form.category) {
      toast.error('يرجى تعبئة الاسم والفئة والسعر على الأقل')
      return
    }
    const data = {
      ...form,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={label}
               className={`glass-card p-5 bg-gradient-to-br ${color} border ${border}`}>
            ...
            <thead>
              <tr className="border-b border-gold-700/20 bg-white/5">
                {['المنتج','الفئة','السعر','الحالة','مميز','إجراءات'].map(h => (
                  <th key={h} className="px-5 py-4 text-right text-gold-500 font-semibold whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-dark-700 rounded-lg flex items-center justify-center text-xl shrink-0">
                          🛡️
                        </div>
                        <div>
                          <p className="text-white font-medium">{p.name}</p>
                          <p className="text-gray-500 text-xs line-clamp-1 max-w-48">{p.description}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      {p.badge
                        ? <span className="text-xs bg-gold-gradient text-dark-900 px-2 py-0.5 rounded-full font-bold">{p.badge}</span>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <motion.button
                          ...
                          onClick={() => openEdit(p)}
                          className="w-8 h-8 rounded-lg bg-blue-900/30 text-blue-400 hover:bg-blue-900/60
                        ...
                        <motion.button
                          ...
                          className="w-8 h-8 rounded-lg bg-red-900/30 text-red-400 hover:bg-red-900/60
                                     flex items-center justify-center transition-colors"
                        >
                          <Trash2 size={14} />
                        </motion.button>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {[
                    { name:'name',  label:'اسم المنتج *',   type:'text', placeholder:'خزينة فايروفايل...' },
                    { name:'price', label:'السعر (ريال) *', type:'number', placeholder:'2800' },
                ...
                <div>
                  ...
                  <input
                    name="features" type="text" value={form.features} onChange={handleChange}
                    placeholder="مقاومة للحريق, قفل إلكتروني, ضمان 3 سنوات"
                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-gold-700/20
                               text-white placeholder-gray-600 focus:outline-none focus:border-gold-500 transition-colors"
                  />
                </div>
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-gold-700/10">
                  ...
                  />
                  <label htmlFor="inStock" className="text-white font-medium cursor-pointer">
                    متوفر في المخزون
                  </label>
                </div>
