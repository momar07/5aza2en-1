...
import { Plus, Pencil, Trash2, X, Save, BookOpen, Clock, User } from 'lucide-react'
...
export default function ManageBlog() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.title || !form.excerpt) {
      toast.error('يرجى إدخال العنوان والمقتطف على الأقل')
      return
    }
    const data = {
      ...form,
  ...
  const handleDelete = (id, title) => {
    if (window.confirm(`حذف مقال "${title}"؟`)) {
      deleteBlog(id)
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={label} className="glass-card p-5 bg-gradient-to-br from-gold-900/30 to-dark-800/50
            ...
            {blogs.map((post, i) => (
              ...
              >
                ...
                <div className="w-16 h-16 bg-dark-700 rounded-xl flex items-center justify-center
                                text-3xl shrink-0 border border-gold-700/10">
                  📝
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    {post.category && (
                      <span className="text-xs bg-gold-900/30 text-gold-400 border border-gold-700/20
                                       px-2 py-0.5 rounded-full">
                        {post.category}
                      </span>
                    )}
                    {post.tags?.slice(0,2).map(t => (
                      <span key={t} className="text-xs text-gray-500 bg-white/5 px-2 py-0.5 rounded-full">
                        #{t}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                    <span className="flex items-center gap-1">
                      <User size={11} className="text-gold-600" /> {post.author}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock size={11} className="text-gold-600" /> {post.readTime}
                    </span>
                    <span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <motion.button
                    ...
                    className="w-9 h-9 rounded-xl bg-blue-900/30 text-blue-400
                               hover:bg-blue-900/60 flex items-center justify-center transition-colors"
                  >
                    <Pencil size={15} />
                  </motion.button>
                  <motion.button
                    ...
                    className="w-9 h-9 rounded-xl bg-red-900/30 text-red-400
                               hover:bg-red-900/60 flex items-center justify-center transition-colors"
                  >
                    <Trash2 size={15} />
                  </motion.button>
                </div>
        ...
        {modal && (
          ...
          >
            <motion.div
              ...
              className="bg-dark-800 border border-gold-700/20 rounded-2xl w-full max-w-2xl
                         max-h-[92vh] overflow-y-auto shadow-gold-lg"
            >
              {/* Header */}
              ...
              {preview ? (
                ...
                <div className="p-6">
                  ...
                  <div
                    className="text-gray-300 leading-loose prose-custom"
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  ...
                  <div>
                    <label className="block text-gold-500 text-sm mb-2">
                      محتوى المقال
                      <span className="text-gray-500 font-normal mr-1">(يدعم HTML)</span>
                    </label>
                    <textarea
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      <input
                        name="readTime" type="text" value={form.readTime} onChange={handleChange}
                        placeholder="5 دقائق"
                        className="w-full px-4 py-3 rounded-xl bg-white/5 border border-gold-700/20
                                   text-white placeholder-gray-600 focus:outline-none focus:border-gold-500 transition-colors"
                      />
